...
const generateMockSafetyRoute = (params: RouteRequestInput): SafetyRoute[] => {
  ...
  const addPathPoints = (latA: number, lngA: number, latB: number, lngB: number, numSteps: number) => {
...
const fetchSafetyRoutes = async (params: RouteRequestInput): Promise<SafetyRoute[]> => {
  ...
  try {
    const response = await navigationRepository.getSafeRoute(params.endLat, params.endLng);
    ...
    if (error.code === "NETWORK_OFFLINE" || !navigator.onLine) {
      const cached = await OfflineStorage.getCachedSpatialData<SafetyRoute[]>(hubCacheKey(params));
      if (cached) {
    ...
    console.warn("API connection failed. Falling back to simulated AI routes:", error);
