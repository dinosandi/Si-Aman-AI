...
const API_BASE_URL =
  import.meta.env.VITE_API_URL || "https://api-siaman.madiunkab.go.id/v1";
...
const handleUnauthorized = () => {
  ...
  localStorage.setItem(
    "auth_flash_message",
    "Sesi telah habis. Silakan melakukan login ulang untuk melanjutkan.",
  );
...
httpClient.interceptors.response.use(
  (response) => {
    ...
    if (
      data &&
      data.success === false &&
      (data.statusCode === 401 || data.status === 401)
    ) {
      const isAuthRequest =
  ...
  (error: AxiosError) => {
    ...
    if (error.response) {
      ...
      if (
        ...
        (error.response.data as any)?.statusCode === 401
      ) {
        if (!isAuthRequest) {
      ...
      customError.message =
        "Koneksi gagal. Periksa koneksi internet Anda (Offline).";
